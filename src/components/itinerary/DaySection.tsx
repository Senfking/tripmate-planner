        ...
        {costPrompt && onCreateExpenseFromItem && (
          <div className="flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/5 px-3 py-2 animate-fade-in">
            ...
            <Button
              ...
              onClick={() => {
                // Resolve item id — for new items, find by title match
                let resolvedId = costPrompt.itemId;
                if (!resolvedId) {
                  const match = items.find(
                    (i) => i.title === costPrompt.title && i.day_date === costPrompt.dayDate
                  );
                  resolvedId = match?.id || "";
                }
                onCreateExpenseFromItem({
                  title: costPrompt.title,
                  date: costPrompt.dayDate,
                  amount: costPrompt.cost.amount,
                  currency: costPrompt.cost.currency,
                  itineraryItemId: resolvedId || undefined,
                });
                const key = costPromptKey(
                  resolvedId || `new:${costPrompt.title}`,
                  costPrompt.title,
                  ""
                );
            ...
            <button
              ...
              onClick={() => {
                const key = costPromptKey(
                  costPrompt.itemId || `new:${costPrompt.title}`,
                  costPrompt.title,
                  ""
                );
                dismissedKeys.current.add(key);

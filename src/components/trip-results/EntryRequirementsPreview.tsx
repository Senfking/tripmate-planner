...
export function EntryRequirementsPreview({
  ...
  if (!user) {
    const ctaClass =
      "group relative block w-full overflow-hidden rounded-2xl border border-[#0D9488]/25 bg-gradient-to-br from-[#0D9488] via-[#0F766E] to-[#134E4A] p-4 text-left shadow-[0_8px_24px_-12px_rgba(13,148,136,0.45)] transition-transform hover:scale-[1.01] active:scale-[0.99]";
    const inner = (
      <>
        ...
        <div className="relative flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-[12px] leading-snug text-white/80">
              Get personalized entry requirements based on your passport — visa rules, validity, and required documents.
            </p>
            <span className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-[12px] font-semibold text-[#0F766E] shadow-sm transition-all group-hover:gap-2">
    ...
    return (
      wrap(
        authGate ? (
          <button type="button" onClick={() => authGate()} className={ctaClass}>
            {inner}
          </button>
        ) : (
          <Link to="/ref" className={ctaClass}>
            {inner}
          </Link>
        ),
      )

...
DECLARE
  ...
  _trip_code text;
  _p_rio uuid := gen_random_uuid(); _p_iguazu uuid := gen_random_uuid();
  _p_floripa uuid := gen_random_uuid(); _p_bsas uuid := gen_random_uuid();
  _do_rio uuid := gen_random_uuid(); _do_iguazu uuid := gen_random_uuid();
  _do_floripa uuid := gen_random_uuid(); _do_bsas uuid := gen_random_uuid();
  _poll1 uuid := gen_random_uuid(); _poll2 uuid := gen_random_uuid(); _poll3 uuid := gen_random_uuid();
  _po1a uuid := gen_random_uuid(); _po1b uuid := gen_random_uuid();
  _po2a uuid := gen_random_uuid(); _po2b uuid := gen_random_uuid();
  _po3a uuid := gen_random_uuid(); _po3b uuid := gen_random_uuid();
  _it1 uuid := gen_random_uuid(); _eid uuid;
BEGIN
  _trip_code := public.generate_trip_code();
  ALTER TABLE trips DISABLE TRIGGER trg_auto_add_trip_owner;
  ALTER TABLE trips DISABLE TRIGGER trg_auto_trip_code;
  ...
  ALTER TABLE trips ENABLE TRIGGER trg_auto_add_trip_owner;
  ALTER TABLE trips ENABLE TRIGGER trg_auto_trip_code;

  INSERT INTO trip_members (trip_id, user_id, role) VALUES (_trip_id, _oliver, 'owner'), (_trip_id, _juntob, 'member');
  ...
  INSERT INTO proposal_reactions (proposal_id, user_id, value) VALUES
    ...
    (_p_floripa, _oliver, 'up'), (_p_floripa, _juntob, 'up'),
  ...
  INSERT INTO polls (id, trip_id, type, title, status) VALUES
    (_poll1, _trip_id, 'preference', 'Boat ride at Iguazu?', 'open'),
    (_poll2, _trip_id, 'preference', 'Extra night in Florianópolis or fly home earlier?', 'open'),
    (_poll3, _trip_id, 'preference', 'Wedding gift — group present or individual?', 'open');
  INSERT INTO poll_options (id, poll_id, label, sort_order) VALUES
    (_po1a, _poll1, 'Yes definitely', 0), (_po1b, _poll1, 'Maybe if not too touristy', 1),
    (_po2a, _poll2, 'Extra night', 0), (_po2b, _poll2, 'Fly home earlier', 1),
    (_po3a, _poll3, 'Group present', 0), (_po3b, _poll3, 'Individual', 1);
  INSERT INTO votes (poll_option_id, user_id, value) VALUES
    (_po1a, _oliver, 'yes'), (_po1a, _juntob, 'yes'), (_po2a, _oliver, 'yes');

  _it1 := gen_random_uuid();
  INSERT INTO itinerary_items (id, trip_id, day_date, start_time, title, location_text, status, created_by, sort_order) VALUES
    ...
    (gen_random_uuid(), _trip_id, '2025-05-23', '08:00', 'Christ the Redeemer — go early before the crowds', 'Corcovado', 'booked', _oliver, 0),
    (gen_random_uuid(), _trip_id, '2025-05-23', '12:00', 'Lunch in Santa Teresa + Selarón steps', 'Santa Teresa', 'planned', _juntob, 1),
    (gen_random_uuid(), _trip_id, '2025-05-23', '17:00', 'Sugarloaf sunset', 'Pão de Açúcar', 'planned', _oliver, 2),
    (gen_random_uuid(), _trip_id, '2025-05-24', '09:00', 'Tijuca rainforest hike', 'Floresta da Tijuca', 'idea', _juntob, 0),
    (gen_random_uuid(), _trip_id, '2025-05-24', '14:00', 'Ipanema beach', 'Ipanema Beach', 'planned', _oliver, 1),
    (gen_random_uuid(), _trip_id, '2025-05-24', '20:00', 'Dinner at Aprazível', 'Santa Teresa', 'idea', _juntob, 2),
    (gen_random_uuid(), _trip_id, '2025-05-25', '12:00', 'Lunch — easy chill day before wedding', 'Leblon', 'planned', _oliver, 0);
  ...
  INSERT INTO itinerary_items (id, trip_id, day_date, start_time, title, location_text, notes, status, created_by, sort_order) VALUES
    (gen_random_uuid(), _trip_id, '2025-05-27', '11:00', 'Late start — recovery beach day', 'Barra da Tijuca', NULL, 'planned', _juntob, 0),
    (gen_random_uuid(), _trip_id, '2025-05-27', '14:00', 'Churrasco lunch', 'Fogo de Chão Rio', NULL, 'planned', _oliver, 1),
    (gen_random_uuid(), _trip_id, '2025-05-27', '19:00', 'Boteco evening', 'Lapa', NULL, 'idea', _juntob, 2),
    (gen_random_uuid(), _trip_id, '2025-05-28', '10:00', 'Fly GIG → IGU', 'Santos Dumont Airport', 'GIG → IGU aim 10:00–13:00 · ~2h15', 'confirmed', _oliver, 0),
    (gen_random_uuid(), _trip_id, '2025-05-28', '14:00', 'Brazil side of the falls', 'Parque Nacional do Iguaçu', NULL, 'booked', _oliver, 1),
    (gen_random_uuid(), _trip_id, '2025-05-28', '19:00', 'Dinner at Porto Canoas restaurant', 'Inside the national park', NULL, 'idea', _juntob, 2),
    (gen_random_uuid(), _trip_id, '2025-05-29', '08:00', 'Argentina side: Devil''s Throat + optional boat ride', 'Parque Nacional Iguazú, Argentina', NULL, 'planned', _juntob, 0),
    (gen_random_uuid(), _trip_id, '2025-05-29', '18:00', 'Fly IGR → FLN', 'Cataratas del Iguazú Airport', 'IGR → FLN aim 18:00–21:00 · ~1h50 via GRU', 'confirmed', _oliver, 1),
    (gen_random_uuid(), _trip_id, '2025-05-30', '09:00', 'Joaquina beach', 'Joaquina', NULL, 'planned', _oliver, 0),
    (gen_random_uuid(), _trip_id, '2025-05-30', '13:00', 'Lunch at Ostradamus — fresh oysters', 'Ribeirão da Ilha', NULL, 'idea', _juntob, 1),
    (gen_random_uuid(), _trip_id, '2025-05-30', '18:00', 'Sunset drinks at the lagoon', 'Lagoa da Conceição', NULL, 'idea', _juntob, 2),
    (gen_random_uuid(), _trip_id, '2025-05-31', '09:00', 'Santo Antônio de Lisboa — colonial village, last coffee', 'Santo Antônio de Lisboa', NULL, 'planned', _oliver, 0),
    (gen_random_uuid(), _trip_id, '2025-05-31', '18:00', 'Fly home FLN → GRU → DXB', 'Hercílio Luz Airport', 'FLN → GRU + GRU → DXB overnight ~14h', 'confirmed', _oliver, 1);
  ...
  INSERT INTO comments (trip_id, itinerary_item_id, user_id, body) VALUES (_trip_id, _it1, _oliver, 'TEST DATA — remove before production');
  RETURN jsonb_build_object('trip_id', _trip_id, 'trip_code', _trip_code);
END;
$fn$;

SELECT public._seed_brazil_trip();
DROP FUNCTION public._seed_brazil_trip();

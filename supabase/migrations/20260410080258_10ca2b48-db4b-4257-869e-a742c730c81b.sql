...
BEGIN
  ...
  FOR _idx IN 0 .. jsonb_array_length(_items) - 1 LOOP
    ...
    FOR _assigned_user_id IN
      SELECT value::uuid
      FROM jsonb_array_elements_text(COALESCE(_assignments -> (_idx::text), '[]'::jsonb)) AS value
    LOOP
      IF NOT EXISTS (
        SELECT 1
      ...
      INSERT INTO public.expense_line_item_claims (line_item_id, user_id)
      VALUES (_line_item_id, _assigned_user_id);
    END LOOP;
  END LOOP;
END;
$$;

    ...
    const extractionPrompt = `Extract booking information from this document. Return ONLY a JSON object with these fields (null for anything not found):
{
  ...
  "departure": "City (IATA) — e.g. 'Dubai (DXB)' or 'London (LHR)'. ALWAYS include the 3-letter airport code in parentheses if it appears anywhere in the document. null if not found.",
  "destination": "City (IATA) — e.g. 'Bali (DPS)' or 'Bangkok (BKK)'. ALWAYS include the 3-letter airport code in parentheses if it appears anywhere in the document. null if not found.",
